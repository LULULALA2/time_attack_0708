...
class JobApply(APIView):
    permission_classes = [permissions.IsCandidateUser]

    def post(self, request):
        job_post = int(request.data.get("job_post", None))
        user_job_post = UserJobPostSerializer(data=request.data)
        user_job_post.save(candidate=request.user, job_post=job_post.id)
